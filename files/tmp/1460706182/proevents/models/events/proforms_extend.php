...
*/
	...
	public function onSubmit($form){
		$question_set = $form->asID;
		$setAttribs = AttributeSet::getByID($question_set)->getAttributeKeys();
		
		foreach ($setAttribs as $ak) {
			if($ak->getAttributeType()->getAttributeTypeHandle() == 'price_event'){
				$db = Loader::db();
				$noPayment = $db->GetOne('select event_require_payment from atPriceEventSettings where akID = ?', $ak->getAttributeKeyID());
				if($noPayment > 0){
					Events::fire('proforms_item_payment',$form);
				}
			}
		}
	}
