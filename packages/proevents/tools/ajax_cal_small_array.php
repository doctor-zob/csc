...
if($_REQUEST['state']){
	$c = Stack::getByID($_REQUEST['cID'],$_REQUEST['state']);
}else{
	$c = Stack::getByID($_REQUEST['cID']);
}
...
$euro_cal = 0;
	...
	print '<h1>'.$_REQUEST['title'].'</h1>' ;
		...
		for($mi=0;$mi<$cal_num;$mi++){
			...
			switch($day_of_week){ 
					case t('Mon') : if($euro_cal >= 1){$blank = 0;}else{$blank = 1;} break; 
					case t('Tue') : if($euro_cal >= 1){$blank = 1;}else{$blank = 2;} break; 
					case t('Wed') : if($euro_cal >= 1){$blank = 2;}else{$blank = 3;} break; 
					case t('Thu') : if($euro_cal >= 1){$blank = 3;}else{$blank = 4;} break; 
					case t('Fri') : if($euro_cal >= 1){$blank = 4;}else{$blank = 5;} break; 
					case t('Sat') : if($euro_cal >= 1){$blank = 5;}else{$blank = 6;} break; 
					case t('Sun') : if($euro_cal >= 1){$blank = 6;}else{$blank = 0;} break; 
			}
							...
							if($date == $daynum){
								...
								if($events_item==$day_num){
									...
									if ($ep->getCollectionAttributeValue('exclude_nav')) {
										...
										print '<h2><a href="'.$url.'?eID='.$eID.'">'.$title.'</a></h2>';
										if($location!=''){	
