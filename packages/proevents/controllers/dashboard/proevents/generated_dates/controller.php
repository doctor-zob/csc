...
class DashboardProeventsGeneratedDatesController extends Controller {
	...
	public function view(){
		...
		if($this->request('search_type')){
			$_SESSION['search_type'] = $this->request('search_type');
		}
		
		if($_SESSION['search']){
			$this->set('fullDateList', $eventify->getSearchEvents($_SESSION['search'],$_SESSION['search_type']));
		}else{
	...
	public function date_edit(){
		...
		$event_price = $this->request('event_price');
		$event_qty = $this->request('event_qty');
	...
	public function delete($eID,$name) {
		...
		$this->set('message', t('"'.$name.'" has been deleted. Remember to edit your event and add this date to the "exclude dates" section.')); 
